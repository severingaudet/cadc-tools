...
def summarize_fields():
    ...
    summary_filename = "byField/fieldSummary.csv"
...
#
    for index, row in field_names.iterrows():
        ...
        field_filename = f"byField/{field}.csv"
...
def process_collInstr():
    obs_filename = "collInstrTotalObs.csv"
    planes_filename = "collInstrTotalPlanes.csv"
    fields_filename = "fieldNames.csv"
    summary_filename = "byCollectionInstrument/collInstrSummary.csv"
...
#
# Now loop through the the list of collection-instrument.
#
    for index, collInstr_row in array_coll_instr_obs.iterrows():
        collection = collInstr_row['collection']
        instrument_name = collInstr_row['instrument_name']
        collInstr_filename = f"{collection}-{instrument_name}.csv" 
        
    
    for index, row in field_names.iterrows():
        field = row['field_name']
        field_filename = f"{field}.csv"
